...
describe("INTERNAL -- streamHtml", function ()
{
    ...
    it("works", function ()
    {
        return streamHtml(conn.absoluteUrl + "/normal/no-links.html", null, helpers.options())
    ...
    {
        return streamHtml(conn.absoluteUrl + "/redirect/redirect.html", null, helpers.options())
            .then(function (result)
            {
                expect(isStream(result.stream)).to.be.true;
    ...
    it("rejects an erroneous url", function ()
    {
        ...
        return streamHtml("/normal/fake.html", null, helpers.options())
            ...
            {
                expect(error).to.be.an.instanceOf(Error);
                //expect(error.message).to.equal("Invalid URL");  // TODO ::
                // https://github.com/joepie91/node-bhttp/issues/4
            })
            .then(function ()
            {
                if (accepted !== false)
