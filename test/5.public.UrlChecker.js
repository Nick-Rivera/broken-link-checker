    ...
    describe("edge cases", function ()
    {
            ...
            new UrlChecker(helpers.options(), {
                link: function (result)
                {
                    expect(result).to.be.like({
                        url: {
                            original: "/normal/no-links.html", resolved: conn.absoluteUrl + "/normal/no-links.html"
                        }, base: {
        ...
        it("supports multiple queue items", function (done)
        {
            ...
            var instance = new UrlChecker(helpers.options(), {
                ...
                {
                    expect(results).to.have.length(3);
                    expect(results).to.be.like([{url: {original: conn.absoluteUrl + "/normal/index.html"}}, {url: {original: conn.absoluteUrl + "/normal/no-links.html"}}, {url: {original: conn.absoluteUrl + "/normal/fake.html"}}]);
                    done();
                }
            });
