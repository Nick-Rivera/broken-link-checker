...
function SiteChecker(options, handlers)
{
    ...
    this.siteUrlQueue = new RequestQueue({
        maxSockets: 1, rateLimit: this.options.rateLimit
    }, {
...
SiteChecker.prototype.enqueue = function (firstPageUrl, customData)
{
    return this.siteUrlQueue.enqueue({
        url: firstPageUrl, data: {customData: customData}
    });
};
...
function maybeEnqueuePage(instance, link, customData)
{
    ...
    if (link.excluded === true)
    {
        ...
        {
            case "BLC_KEYWORD":
            case "BLC_ROBOTS":  // TODO :: catches rel=nofollow links but will also catch meta/header excluded links --
                                // fine?
            case "BLC_SCHEME":
            {
                return false;
    ...
    if ((attrSupported !== true) || (link.broken === true) || (link.internal !== true) || (instance.sitePagesChecked.get(link.url.resolved) === true) || (isAllowed(instance, link) === false))
    {
        return false;
    }
